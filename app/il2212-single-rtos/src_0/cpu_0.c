		...
		if(PERFORMANCE){
			...
			"Total time", "Time per iteration"        // Display-name of section(s).
			   ...
			   double total_time =  (double)perf_get_section_time(PERFORMANCE_COUNTER_0_BASE, SECTION_1)/(double)alt_get_cpu_freq();
			    double normalized_time = total_time/no_of_imgs;
      	...
      	//printf("\nThroughtput=%f img/s", (double)perf_get_section_time(PERFORMANCE_COUNTER_0_BASE, SECTION_1)/(double)alt_get_cpu_freq(); );
		}
		else{
